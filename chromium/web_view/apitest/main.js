...
function testAutosizeHeight () {
  ...
  webview.addEventListener('sizechanged', function (e) {
    ...
    if (step == 1) { webview.maxheight = 50 }

    // We are done once the size settles on the final width and height.
    if (e.newHeight == finalHeight && e.newWidth == finalWidth) { embedder.test.succeed() }
    ++step
  })
...
function testLoadAbortNonWebSafeScheme () {
  const webview = document.createElement('webview')
  const chromeGuestURL = 'chrome-guest://abc123/'
  webview.addEventListener('loadabort', function (e) {
    embedder.test.assertEq('ERR_DISALLOWED_URL_SCHEME', e.reason)
    ...
    webview.captureVisibleRegion(
      ...
      function (imgdata) {
        if (chrome.runtime.lastError) {
          ...
          if (!imgdata.startsWith('data:image/jpeg;base64')) { console.log('imgdata = ' + imgdata) }
...
embedder.test.testList = {
  ...
  testLoadAbortUnknownScheme,
  testLoadEventsSameDocumentNavigation,
  ...
  testWebRequestAPIWithExtraHeaders,
  testWebRequestAPIExistence,
  testWebRequestAPIGoogleProperty,
  testCaptureVisibleRegion,
  testClosedShadowRoot
}
