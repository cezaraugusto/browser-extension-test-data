...
let deviceChangedListener = null
...
chrome.test.runTests([
  ...
  function getActiveDevicesTest () {
    chrome.audio.getDevices(chrome.test.callbackPass(function (initial) {
      const initialActiveDevices = initial.filter(function (device) {
      ...
      const initialActiveInputs = initialActiveDevices.filter(function (device) {
      ...
      const initialActiveOutputs = initialActiveDevices.filter(function (device) {
  ...
  function getInactiveDevicesTest () {
    chrome.audio.getDevices(chrome.test.callbackPass(function (initial) {
      const initialInactiveDevices = initial.filter(function (device) {
        return !device.isActive
      })
  ...
  function setPropertiesTest () {
    chrome.audio.getDevices(chrome.test.callbackPass(function (initial) {
      const expectedDevices = deviceListToExpectedDevicesMap(initial)

      // Update expected input devices with values that should be changed in
      // test.
      const updatedInput = expectedDevices['40002']
      ...
      // test.
      const updatedOutput = expectedDevices['30001']
  ...
  function inputMuteTest () {
    const getMute = function (callback) {
  ...
  function setActiveDevicesOutputOnlyTest () {
    chrome.audio.getDevices({
      ...
      const initialActiveInputs = getDeviceIds(initial)
      chrome.test.assertTrue(initialActiveInputs.length > 0)
      ...
      chrome.audio.setActiveDevices({
        ...
        chrome.audio.getDevices({
          ...
          const expected = ['30003'].concat(initialActiveInputs).sort()
  ...
  function setActiveDevicesFailInputTest () {
    chrome.audio.getDevices({
      ...
      const initialActiveIds = getDeviceIds(initial)
  ...
  function deprecated_setProperties_volume () {
    chrome.audio.getDevices(chrome.test.callbackPass(function (initial) {
      const expectedDevices = deviceListToExpectedDevicesMap(initial)
      const expectedError = '|volume| property is deprecated, use |level|.'
  ...
  function deprecated_setProperties_gain () {
    chrome.audio.getDevices(chrome.test.callbackPass(function (initial) {
      const expectedDevices = deviceListToExpectedDevicesMap(initial)
      const expectedError = '|gain| property is deprecated, use |level|.'
