...
chrome.test.runTests([
  ...
  function setPropertiesTest () {
    chrome.audio.getInfo(chrome.test.callbackPass(function (
      ...
      const expectedInput = deviceListToExpectedDevicesMap(initialInput)
      const expectedOutput = deviceListToExpectedDevicesMap(initialOutput)

      // Update expected input devices with values that should be changed in
      // test.
      const updatedInput = expectedInput['40002']
      ...
      // test.
      const updatedOutput = expectedOutput['30001']
  ...
  function setPropertiesInvalidValuesTest () {
    chrome.audio.getInfo(chrome.test.callbackPass(function (
      ...
      const expectedOutput = deviceListToExpectedDevicesMap(initialOutput)
      const expectedInput = deviceListToExpectedDevicesMap(initialInput)
      const expectedError = 'Could not set volume/gain properties'
